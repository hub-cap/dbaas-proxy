...
LoadBalancerFlow.prototype._provisionLoadBalancer = function (callback) {
  ...
  var remoteLoadBalancer = {
    name: 'dbaas-proxy-' + this.state.childLoadBalancer.instanceUUID,
...
LoadBalancerFlow.prototype._savePkgCloudLB = function (callback,
                                                       err, pkgCloudLB) {
  this.state.remoteLoadBalancer = {id: pkgCloudLB.id};
  callback(err);
};
...
LoadBalancerFlow.prototype._saveParentPkgCloudLB = function (callback,
  ...
  this.state.parentLoadBalancer = {id: pkgCloudLB.id,
                                   virtualIP: ipv4_ips[0].id};
  callback(err);
};

LoadBalancerFlow.prototype._saveParentLoadBalancer = function (callback) {
  MYSQL_CLIENT.createActiveLoadBalancer(this.state.parentLoadBalancer.id,
                                        this.state.parentLoadBalancer.virtualIP,
                                        callback);
};

LoadBalancerFlow.prototype.deleteLoadBalancer = function (instanceUUID,
                                                          callback) {
  this.state.childLoadBalancer = {instanceUUID: instanceUUID};
  async.series([
    this._retrieveLoadBalancerNode.bind(this),
    this._deprovisionLoadBalancer.bind(this),
    this._deleteLoadBalancerNode.bind(this)
  ], this._completedFlow.bind(this, callback));
};

LoadBalancerFlow.prototype._retrieveLoadBalancerNode = function (callback) {
  var self = this;
  MYSQL_CLIENT.retrieveLoadBalancerNode(
    this.state.childLoadBalancer.instanceUUID,
    function (err, results) {
      if (err) {
        LOGGER.error("Could not find a lod balancer with",
                    this.state.childLoadBalancer.instanceUUID);
        LOGGER.error(err);
        callback(err);
        return;
      ...
      if (results.length === 0) {
        ...
        // There should only be one LB w/ a given UUID
        var nodeLB = results[0];
        self.state.childLoadBalancer.loadBalancerID = nodeLB.load_balancer_id;
        self.state.childLoadBalancer.id = nodeLB.id;
        callback(null);
      }
    }
  );
...
LoadBalancerFlow.prototype._deleteLoadBalancerNode = function (callback) {
  MYSQL_CLIENT.deleteLoadBalancerNode(this.state.childLoadBalancer.id);
  callback(null);
};
