...
var RedisInstanceFlow = require('./redis-helper').RedisInstanceFlow;
...
LoadBalancerFlow.prototype.createLoadBalancer = function (privateIP,
  ...
  async.series([
    ...
    this._pollLoadBalancerActive.bind(this),
    this._addDNSRecord.bind(this),
    this._updateLoadBalancer.bind(this),
    this._configureRedisMaster.bind(this),
...
LoadBalancerFlow.prototype._addDNSRecord = function (callback) {
  var self = this;
  LOGGER.debug("adding dns record");
...
LoadBalancerFlow.prototype._configureRedisMaster = function (callback){
  new RedisInstanceFlow().configureMaster(
    this.state.childLoadBalancer.privateIP,
    this.state.childLoadBalancer.privatePort,
    this.state.childLoadBalancer.password,
    this.state.childLoadBalancer.masterPassword,
    callback
  );
};
...
LoadBalancerFlow.prototype._provisionLoadBalancer = function (callback) {
  ...
  var remoteLoadBalancer = {
    ...
    accessList: [{address: '0.0.0.0/0', type: 'DENY'}],
    nodes: [{
...
LoadBalancerFlow.prototype._pollLoadBalancerActive = function (callback) {
  var self = this;
  var timer = setInterval(function() {
    self.pkgCloudClient.getLoadBalancer(
      self.state.remoteLoadBalancer.id,
      function(err, loadBalancer) {
        if (loadBalancer.status != 'BUILD') {
          self.state.childLoadBalancer.status = loadBalancer.status;
          clearInterval(timer);
          callback(null);
        }
      });
...
LoadBalancerFlow.prototype.createSecondaryNode = function (vipUUID,
  ...
  async.series([
    ...
    this._configureRedisSlave.bind(this),
    this._updateSecondaryNode.bind(this)
  ], this._completedFlow.bind(this, callback));

};

LoadBalancerFlow.prototype._configureRedisSlave = function (callback) {
  var masterNode = this.state.nodes.filter(function(n) { return (n.type === 1); })[0];
  if (masterNode === null) {
    callback("Could not find a primary node for the slave");
    return;
  }

  new RedisInstanceFlow().configureSlave(
    this.state.childLoadBalancer.privateIP,
    this.state.childLoadBalancer.privatePort,
    this.state.childLoadBalancer.password,
    this.state.childLoadBalancer.masterPassword,
...
LoadBalancerFlow.prototype._retrieveChildLoadBalancer = function (callback) {
  var self = this;
  MYSQL_CLIENT.retrieveLoadBalancerDetails(
    this.state.childLoadBalancer.vipUUID,
    function (err, results) {
      ...
      if (results.length === 0) {
        callback("No load balancer was found given " +
                 self.state.childLoadBalancer.vipUUID);
      } else {
        ...
        self.state.childLoadBalancer.address = childLoadBalancer.address;
        self.state.childLoadBalancer.dnsName = childLoadBalancer.dns_name;
        self.state.childLoadBalancer.port = childLoadBalancer.port;
        callback(null);
      }
    }
  );
...
LoadBalancerFlow.prototype._retrieveLoadBalancerNodes = function (callback) {
  ...
  MYSQL_CLIENT.retrieveLoadBalancerNodes(
    ...
    function (err, results) {
      ...
      if (results.length === 0) {
        callback("Could not find a load balancer with id " +
                 self.state.childLoadBalancer.vipUUID);
      } else {
...
LoadBalancerFlow.prototype.retrieveLoadBalancerDetails = function (vipUUID,
                                                                   callback) {
  this.state.childLoadBalancer = {vipUUID: vipUUID};
  async.series([
    this._retrieveChildLoadBalancer.bind(this),
    this._retrieveProvisionedLoadBalancerStatus.bind(this)
  ], this._completedFlow.bind(this, callback));

};

LoadBalancerFlow.prototype._retrieveProvisionedLoadBalancerStatus = function (callback) {
  var self = this;
  self.pkgCloudClient.getLoadBalancer(
    self.state.childLoadBalancer.loadBalancerID,
    function(err, loadBalancer) {
      if (err === null) {
        self.state.childLoadBalancer.status = loadBalancer.status;
      } else {
...
LoadBalancerFlow.prototype.getAccessRules = function (vipUUID, callback) {
  this.state.childLoadBalancer = { vipUUID: vipUUID, };

  async.series([
    this._retrieveChildLoadBalancer.bind(this),
    this._retrieveAccessRuleList.bind(this),
  ], callback);
};

LoadBalancerFlow.prototype._retrieveAccessRuleList = function(callback) {
  util.retryCall(10, this.pkgCloudClient,
                 this.pkgCloudClient.getAccessList,
                 this.state.childLoadBalancer.loadBalancerID,
                 callback);
};

LoadBalancerFlow.prototype.addAccessRule = function(vipUUID,
                                                    address,
                                                    callback) {
  this.state.childLoadBalancer = { vipUUID: vipUUID,
                                   accessList: {
  ...
  async.series([
    this._retrieveChildLoadBalancer.bind(this),
    this._provisionNewAccessRule.bind(this),
  ], this._completedFlow.bind(this, callback));
};

LoadBalancerFlow.prototype._provisionNewAccessRule = function (callback) {
  util.retryCall(10, this.pkgCloudClient,
                 this.pkgCloudClient.addAccessList,
                 this.state.childLoadBalancer.loadBalancerID,
                 this.state.childLoadBalancer.accessList,
                 callback);
};

LoadBalancerFlow.prototype.deleteAccessRule = function (vipUUID, accessListID,
                                                        callback) {
  this.state.childLoadBalancer = { vipUUID: vipUUID,
    ...
    async.series([
    this._retrieveChildLoadBalancer.bind(this),
    this._deleteAccessRule.bind(this),
  ], this._completedFlow.bind(this, callback));
};

LoadBalancerFlow.prototype._deleteAccessRule = function (callback) {
  util.retryCall(10, this.pkgCloudClient,
                 this.pkgCloudClient.deleteAccessListItem,
                 this.state.childLoadBalancer.loadBalancerID,
                 this.state.childLoadBalancer.accessListID,
                 callback);
};
