...
MySQL.prototype.updateChildLoadBalancer = function (ID, loadBalancerID, dns, state) {
  this.connection.query(
    'update child_load_balancers set load_balancer_id = ?, state = ?, dns_name = ?  where id = ?',
    [loadBalancerID, state, dns, ID]
  );
};
...
MySQL.prototype.retrieveLoadBalancerDetails = function (id, callback) {
  this.connection.query(
    'select p.address, c.* '+
      'From parent_load_balancers p, child_load_balancers c, load_balancer_nodes n ' +
    'where p.load_balancer_id = c.parent_load_balancer_id and c.id = ?',
    [id],
    callback
  );
};
